...
//  console.log(usersData);
  ...
  const onClickRemove = (id) => {
	if (window.confirm("정말 삭제하시겠습니까?")) {
      ...
      window.location.replace("/notice/"+match.params.page);
	}    
  }
...
<br/>
	  ...
	  <hr/>
      <button className='grayBtn' onClick={() => window.location.replace("/notice/"+match.params.page)}>목록</button>
	{myData == 1 && <>
	  <Link to={`/notice/NoticeEdit/${pid}/${match.params.page}`}>
