...
const PromotionCheck = ({ match, history }) => {
  // 로그인체크
  ...
  const answerRef = firebase.database().ref('promotionCheckList');
...
//  console.log(chkData);
  ...
  const onClickAdd = () => {
    ...
    ToastsStore.success("인증을 완료했습니다.");
...
<br/><pre>{data.content}</pre></p>
	  ...
	  <hr/>
      <button className='grayBtn' onClick={() => history.goBack()}>뒤로가기</button>
	
		<div className="answer_layout">
		<div className="answer_board">
			<p className="comments_title">인증하기</p>
			{answerDatas?.map(answer => <div key={answer.id}>
			{originEmail == ss_account || answer.email == ss_account ?
