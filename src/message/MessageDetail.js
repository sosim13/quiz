...
const MessageDetail = ({ match, history }) => {
  // 로그인체크
  ...
  const answerRef = firebase.database().ref('messageAnswerList');
...
//  console.log(usersData);
  ...
  const onClickRemove = (id) => {
	if (window.confirm("정말 삭제하시겠습니까?")) {
      ...
      window.location.replace("/message/"+match.params.page);
	}    
  }
...
<br/>
		...
		<img src={data.url} width="100%"/>
			<pre width="100%">{data.content}</pre>
        </p>
			<br/>
        </div>
	  <hr/>
      <button className='grayBtn' onClick={() => window.location.replace("/message/"+match.params.page)}>목록</button>
	{myData == 1 && <>
	  <Link to={`/message/MessageEdit/${pid}/${match.params.page}`}>
